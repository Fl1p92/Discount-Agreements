...
class Agreement(models.Model):
    ...
    def __str__(self):
        return f'Agreement concluded with {self.company.title} id# {self.id}'
...
class Period(models.Model):
    ...
    def clean(self, **kwargs):
        ...
        if self.start_date < self.agreement.start_date:
            raise ValidationError({'start_date': 'Period start date must be later than start date of the agreement'})
        # stop_date check
        if self.stop_date > self.agreement.stop_date:
            raise ValidationError({'stop_date': 'Period stop date must be earlier than stop date of the agreement'})
        ...
        if periods.filter(start_date__lt=self.start_date, stop_date__gt=self.start_date).exists():
            raise ValidationError({'start_date': 'Period start date should not intersect with existing periods'})
        if periods.filter(start_date__lt=self.stop_date, stop_date__gt=self.stop_date).exists():
            raise ValidationError({'stop_date': 'Period stop date should not intersect with existing periods'})
    ...
    def save(self, **kwargs):
        self.clean()  # guaranteed validation before save()
        super().save(**kwargs)
...
class Company(models.Model):
    ...
    def __str__(self):
        return f'{self.title} from {self.country.name} id# {self.id}'
...
class Country(models.Model):
    ...
    def __str__(self):
        return f'[{self.code}] {self.name} id# {self.id}'
